...
4. Ablation test — validate attributions by masking top tokens

Produces detailed multi-panel visualizations and cross-seed comparisons.
Results and plots saved to outputs/<run_dir>/xai_analysis/
"""
...
def select_examples(run_dir, n_per_seed=1):
    ...
    for fname in sorted(os.listdir(revised_dir)):
        ...
        # ties on harmbench_score (which is common since the classifier gives
        # 1.00 to any clear jailbreak). We sort by (-harmbench_score, -length
        # difference from seed) so longer rewrites win ties over near-identical
        # typo mutations.
        def sort_key(jb):
...
def analyze_example(model, tokenizer, example, comply_ids, refuse_ids, out_dir, idx):
    ...
    logger.info("  [1/4] Integrated Gradients...")
    t0 = time.time()
    try:
    ...
    logger.info("  [2/4] Divergence heatmap...")
    t0 = time.time()
    try:
    ...
    logger.info("  [3/4] Activation patching...")
    t0 = time.time()
    try:
    ...
    try:
        ...
        clean_co = ll_clean["crossover_layer"]
        jb_co = ll_jb["crossover_layer"]
        logger.info(f"    Clean crossover: L{clean_co}, Jailbreak crossover: L{jb_co} ({time.time()-t0:.1f}s)")
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Run XAI analysis on jailbreak pairs")
    parser.add_argument("run_dir", help="Fuzzer run directory (e.g., outputs/fuzzer_170260)")
    parser.add_argument("--n-per-seed", type=int, default=1,
                        help="Number of examples to analyze per seed")
    parser.add_argument("--out-subdir", default="xai_analysis",
                        help="Name of the output subdirectory under run_dir "
                             "(default: xai_analysis)")
    ...
    # Get compliance/refusal token IDs
    ...
    try:
        plot_cross_seed_divergence(all_results,
                                   os.path.join(out_dir, "cross_seed_divergence.png"))
        logger.info("  Saved cross_seed_divergence.png")
    except Exception as e:
        logger.error(f"  Cross-seed divergence plot failed: {e}", exc_info=True)

    try:
        plot_cross_seed_summary(all_results,
                                os.path.join(out_dir, "cross_seed_layer_importance.png"))
        logger.info("  Saved cross_seed_layer_importance.png")
    except Exception as e:
        logger.error(f"  Cross-seed summary plot failed: {e}", exc_info=True)

    try:
        plot_compliance_score_landscape(all_results,
                                        os.path.join(out_dir, "compliance_landscape.png"))
        logger.info("  Saved compliance_landscape.png")
    except Exception as e:
        logger.error(f"  Compliance landscape plot failed: {e}", exc_info=True)

    try:
        plot_cross_seed_logit_lens(all_results,
                                    os.path.join(out_dir, "cross_seed_logit_lens.png"))
        logger.info("  Saved cross_seed_logit_lens.png")
    except Exception as e:
        logger.error(f"  Cross-seed logit lens plot failed: {e}", exc_info=True)
