...
def plot_token_heatmap(attr_dict, title, save_path, max_tokens=300):
    ...
    # The full-prompt L1 normalization in compute_attribution leaves user-content
    # values tiny when system-prompt tokens dominate the magnitudes.
    max_abs = max(np.abs(scores).max(), 1e-10)
    norm_scores = scores / max_abs

    # Estimate height from token count so the figure isn't squashed for long prompts.
    n_tokens = len(tokens)
    ...
    # Single-line title (truncated with ellipsis). The full prompt is already
    # rendered as colored tokens below, so we don't need to repeat it verbatim.
...
def _plot_ig_comparison(ax, ig_clean, ig_jb, n_show=15):
    ...
    def get_top(attr_dict, n):
        ...
        # Restrict to user-supplied content so the bar chart doesn't get
        # dominated by Mistral system-prompt boilerplate tokens.
        span = attr_dict.get("user_span")
        if span is not None:
            start, end = span
        ...
        # Sort by actual score value for display
    ...
    # Pad the shorter list with empty entries so both rows align on the y-axis.
    ...
    y_j = np.arange(n_rows)

    width = 0.35

    # Clean prompt tokens (left side)
    ...
    ax.barh(y_j - width / 2, scores_j, width, color=colors_j, alpha=0.9, label="Jailbreak (complied)")
...
def _plot_divergence_detailed(ax, divergence):
    ...
    # Highlight top-3 divergent layers
...
def _plot_patching_detailed(ax, patching):
    ...
    bars = ax.bar(range(n), effects, color=[cmap(norm(abs(e))) for e in effects],
...
def plot_cross_seed_divergence(all_results, save_path):
    ...
    n_layers = div_matrix.shape[1]

    # Top panel: Divergence heatmap
    ax1 = axes[0]
    ...
    # Bottom panel: Patching causal effect heatmap
    ax2 = axes[1]
...
def plot_cross_seed_summary(all_results, save_path):
    ...
    for r in all_results:
        ...
        # Accumulate absolute causal effect
    ...
    # Add early/middle/late annotations
    thirds = n_layers // 3
...
def plot_logit_lens_single(ll_result, title, save_path):
    ...
    # Top: comply/refuse probability curves
    ax1 = axes[0]
    ...
    # Annotate top-1 predicted token at selected layers
